...
row = 1-3;
for rc = 1:length(roi)
    % [~,col] = tilerowcol(hT,hA{end}.Layout.Tile);
    % col = col + hA{end}.Layout.TileSpan(2);
    % row = 1;
    ...
    for r = 1:length(roi{rc})
        % if strcmp(roi{rc}(r).label,'vesselAll'); continue; end;
        % if row>16-3; col = col+3; row = 1; end
        if col>38-3; row = row+3; col = col0; end
        skipThis = ~isfield(roi{rc},'im') || isempty(roi{rc}(r).im);

        hA{end+1} = nexttile(tilenum(hT,row,col),[3 3]);
        if ~skipThis
            imagesc(roi{rc}(r).im.base.x,roi{rc}(r).im.base.y,roi{rc}(r).im.base.im,cLim); colormap(hA{end},'gray'); hA{end}.DataAspectRatio = [1 1 1];
        end
