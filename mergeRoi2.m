function roiGrpMrgd = mergeRoi2(roiGrp,commonFrameFlag)
    ...
    for g = 1:length(roiGrp)
        ...
        % merge data fields
        fieldList = fields(roiGrp{g}(1).mt);
        for fi = 1:length(fieldList)
            sz = size(roiGrp{g}(1).mt.(fieldList{fi}).vec,1:8); sz(6) = length(roiGrp{g});
            roiGrpMrgd{g}.mt.(fieldList{fi}).vec = zeros(sz);
            for r = 1:length(roiGrp{g})
                roiGrpMrgd{g}.mt.(fieldList{fi}).vec(:,:,:,:,:,r,:,:) = mean(roiGrp{g}(r).mt.(fieldList{fi}).vec,6);
            end

            % copy info fields
            fieldList2 = fields(roiGrp{g}(1).mt.(fieldList{fi}));
            fieldList2(ismember(fieldList2,{'vec' 'vecAv' 'vecEr'})) = [];
            for ffi = 1:length(fieldList2)
                roiGrpMrgd{g}.mt.(fieldList{fi}).(fieldList2{ffi}) = roiGrp{g}(1).mt.(fieldList{fi}).(fieldList2{ffi});
            end

            roiGrpMrgd{g}.mt.psd.info = strsplit(roiGrpMrgd{g}.mt.psd.info,' x ');
            roiGrpMrgd{g}.mt.psd.info{6} = 'roi';
            roiGrpMrgd{g}.mt.psd.info = strjoin(roiGrpMrgd{g}.mt.psd.info,' x ');
        end
    end
