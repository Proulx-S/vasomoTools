function [out1, out2, out3, out4, info] = volTsGetResp3(do,info,volTs,dsgn,volAnat,force,verbose)
global srcAfni
...
if isnan(param.nDummy); param.nDummy = 0; end
if isnan(param.nDummyRemoved); param.nDummyRemoved = 0; end
...
switch info.dataSetLabel
    case {'vsmDriven' 'vsmRing' 'zhangxuanPC'}
        if isfield(dsgn,'nullTrial') && ~isempty(dsgn.nullTrial)
            ...
            dsgn.condList = ones(size(dsgn.onsetList));
            dsgn.condLabelList = {'stim'}';
        end
        ...
        if isMRI(mask)
            ...
            [volResp,volRespCat,~,param_getResp] = getAct2(volTs,dsgn,mask,param,forceThis,verboseThis);
        end
