function [files,fRun,fSes,fSes_echoCat,param] = getRespTmp(volTs,dsgn,fMask,param,force,verbose)
fSes_echoCat = [];
...
param.funDsgn.k        = 1;
param.funDsgn.startSeq = dsgn.onsetList(~dsgn.nullTrial);
param.funDsgn.durSeq   = dsgn.ondurList(~dsgn.nullTrial);
param.funDsgn.condSeq  = ones(size(param.funDsgn.startSeq));
param.funDsgn.label    = dsgn.label;
param.funDsgn.trDecon  = param.trDecon;


param.funDsgn(end+1).trStim   = dsgn.dt;
param.funDsgn(end).k        = 2;
param.funDsgn(end).startSeq = dsgn.onsetList(dsgn.nullTrial);
param.funDsgn(end).durSeq   = dsgn.ondurList(dsgn.nullTrial);
param.funDsgn(end).condSeq  = 2.*ones(size(param.funDsgn(end).startSeq));
param.funDsgn(end).label    = [dsgn.label 'CATCH'];
param.funDsgn(end).trDecon  = param.trDecon;
...
% Functional design %
%%%%%%%%%%%%%%%%%% %%
funDsgnCatch = param.funDsgn(2); param.funDsgn(2) = [];
k = param.funDsgn.k;
...
for E = 1:size(fList,2)
    for I = 1:size(fList,1)
        ...
        % if ~exist(fStat,'file') || force
            ...
            fStimCatch = replace(fStim,'cond-visOn',['cond-' funDsgnCatch.label]);
            fStim     = replace(fStim,'cond-visOn',['cond-' param.funDsgn.label]);
            startSeqCatch = funDsgnCatch.startSeq;
            condSeqCatch  = funDsgnCatch.condSeq;
            
            fRespCatch  = replace(fResp ,'cond-visOn',['cond-' funDsgnCatch.label]);
            fResp       = replace(fResp ,'cond-visOn',['cond-' param.funDsgn.label]);
            fFitCatch   = replace(fFit  ,'cond-visOn',['cond-' funDsgnCatch.label]);
            fFit        = replace(fFit  ,'cond-visOn',['cond-' param.funDsgn.label]);
            fResidCatch = replace(fResid,'cond-visOn',['cond-' funDsgnCatch.label]);
            fResid      = replace(fResid,'cond-visOn',['cond-' param.funDsgn.label]);
            fStatCatch  = replace(fStat ,'cond-visOn',['cond-' funDsgnCatch.label]);
            fStat       = replace(fStat ,'cond-visOn',['cond-' param.funDsgn.label]);
            
            fMat = replace(fMat ,'cond-visOn','cond-all');
            
            [tmp,n] = afniCmd(fIn,fMask,{fStim fStimCatch},param.nDummy,param.tr,{startSeq startSeqCatch},{condSeq condSeqCatch},trDecon,{param.funDsgn.label funDsgnCatch.label},TENTzeroFlag,{fResp fRespCatch},{fFit fFitCatch},{fResid fResidCatch},fMat,{fStat fStatCatch},verbose,param.nDummyRemoved);
...
function [cmd,n] = afniCmd(fIn,fMask,fStimAll,nDummy,tr,startSeqAll,condSeqAll,trDecon,labelAll,TENTzeroFlag,fRespAll,fFitAll,fResidAll,fMat,fStatAll,verbose,nDummyRemoved)
...
cmd{end+1} = ['-num_stimts ' num2str(length(fStimAll)) ' \'];

deconWin = min(diff(sort([startSeqAll{:}])));
...
for k = 1:length(fStimAll)
    fStim    = fStimAll{k};
    startSeq = startSeqAll{k};
    condSeq  = condSeqAll{k};
    label    = labelAll{k};
    fResp    = fRespAll{k};
    fFit     = fFitAll{k};
    fResid   = fResidAll{k};
    ...
    cmd{end+1} = ['-stim_label ' num2str(k) ' ' char(label) ' \'];
