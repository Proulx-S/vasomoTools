function funPsd = runFullMT4(funTs,W,K,win,onsets,ondurs,mask,extra,skipSVD,skipPSD,verbose,taperPerm,phaseRand,testFlag)
...
if isempty(onsets)
    ...
    if isfield(funTs(1),'dsgn')
        if isfield(funTs(1).dsgn,'onsets')
            onsets = funTs(1).dsgn.onsets;
        else
            onsets = funTs(1).dsgn.onsetList;
        end
    elseif isfield(funTs(1),'ts') && isfield(funTs(1).ts,'dsgn')
        onsets = funTs(1).ts.dsgn.onsetList;
    end
end
...
if isempty(ondurs)
    ...
    elseif isfield(funTs(1),'ts') && isfield(funTs(1).ts,'dsgn')
        ondurs = funTs(1).ts.dsgn.ondurList;
    end
end
...
elseif isstruct(funTs)
    ...
    for I = 1:numel(funTs)
        cohFperm = [0 1.1];
        if isMRI(funTs(I))
            funPsd(I) = doIt(funTs(I)    ,W,K,win,onsets,ondurs,mask,extra,skipSVD,skipPSD,verbose,taperPerm,phaseRand,[],cohFperm,testFlag);
        elseif isfield(funTs,'chanLabel')
            funPsd(I) = doIt(funTs(I)  ,W,K,win,onsets,ondurs,mask,extra,skipSVD,skipPSD,verbose,taperPerm,phaseRand,[],cohFperm,testFlag);
        elseif isfield(funTs,'mri')
            funPsd(I) = doIt(funTs(I).mri,W,K,win,onsets,ondurs,mask,extra,skipSVD,skipPSD,verbose,taperPerm,phaseRand,[],cohFperm,testFlag);
        elseif isfield(funTs,'ts')
            funPsd(I) = doIt(funTs(I).ts,W,K,win,onsets,ondurs,mask,extra,skipSVD,skipPSD,verbose,taperPerm,phaseRand,[],cohFperm,testFlag);
        end
...
function funPsd = doIt(funTs,W,K,win,onsetList,durList,mask,extra,skipSVD,skipPSD,verbose,taperPerm,phaseRand,cohF,cohFperm,testFlag)
tpFlag = false;
