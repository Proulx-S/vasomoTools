function [files,f,fSes,fSes_echoCat,param] = getResp3(volTs,dsgn,mask,param,force,verbose)
...
if isempty(mask)
    if isfield(volTs,'vol2vec')
        mask = [tempname '.nii.gz'];
        mask.mri = volTs(1);
        mask.mri.fspec = mask;
        mask.mri.vol = mask.mri.vol2vec;
        MRIwrite(mask.mri,mask.mri.fspec)
    end
elseif isMRI(mask)
    mask = mask.fspec;
elseif isnumeric(mask) || islogical(mask)
    if isfield(volTs(1),'vec'); mri = rmfield(volTs(1),'vec'); else; mri = volTs(1); end
    mri.vol = mask; mri.fspec = [tempname '.nii.gz'];
...
if isfield(dsgn,'condList')
    param.funDsgn.condSeq  = dsgn.condList;
else
    param.funDsgn.condSeq  = ones(size(param.funDsgn.startSeq));
end
if isa(dsgn,'dsgnInfo')
    param.funDsgn.label    = dsgn.condLabelList;
else
    if isfield(dsgn,'condLabelList')
        param.funDsgn.label    = dsgn.condLabelList;
    else
        param.funDsgn.label    = dsgn.label';
    end
end
param.funDsgn.k        = length(param.funDsgn.label);
param.funDsgn.trDecon  = param.trDecon;


%% Run afni's 3dDeconvolve
[f,param] = runAfni(fVolTs,param,mask,force,verbose); % analysis performed on each echoe within that function
nEcho = size(f,2);
nRunIn = size(fVolTs,1);
nRunOut = size(f,1);
nCond = size(f(1).fResp,2);
...
for R = 1:nRunOut
    for E = 1:nEcho+1
        ...
        if E>nEcho
            ...
            fResp = f(R,E).fResp;
            fRespOnBase = replace(fResp,'_resp.nii.gz','_respOnBase.nii.gz');
            fBase = replace(f.fStat,'_stats.nii.gz','_base.nii.gz');
            f(R,E).fBase = fBase;
            fStat = f(R,E).fStat;
            f(R,E).fRespOnBase = fRespOnBase;
            % if useFittedBaseline
            % Use fitted baseline
            % else
            %     % Use plain average as baseline
        ...
        buck = num2str(1:nRunIn,'Run#%iPol#0_Coef,'); buck(end) = [];
        % for r = 1:nRunIn
        ...
        for c = 1:nCond
            ...
            cmd{end+1} = ['-prefix ' fRespOnBase{c} ' \'];
            cmd{end+1} = ['-a ' fBase ' \'];
            cmd{end+1} = ['-b ' fResp{c} ' \'];
            cmd{end+1} = '-expr ''a+b''';
        end
...
%%% Whole session
if nRunOut>1 && ~isempty(fSes) && ~param.skipCat
    dbstack; error('double-check that')
    for E = 1:nEcho+1
        %%% Define file names
        if E>nEcho
...
if movieFlag
    ...
    for R = 1:nRunOut
        ...
        for c = 1:nCond
            ...
            disp([' run' num2str(R) '/' num2str(nRunOut) '; cond' num2str(c) '/' num2str(nCond)])
            % disp(['  file' num2str(E) '/' num2str(size(f,2)+1)])
            % if E>nEcho
            ...
            % else
            fIn = f(R,E).fRespOnBase{c};
            fOut = replace(fIn,'.nii.gz','');
            f(R,E).fRespOnBaseMovie{c} = [fOut '.avi'];
            f(R,E).fRespOnBaseMovieHighBit{c} = [fOut '.mj2'];
            % end
            ...
            if forceThis || ~exist([fOut '.avi'],'file') || ~exist([fOut '.mj2'],'file')
                ...
                open(vOutHighBit)
                for L = 1:nLoop
                    for frame = 1:resp.nframes
                        writeVideo(vOut,resp.vol(:,:,1,frame));
                        writeVideo(vOutHighBit,uint16(resp.vol(:,:,1,frame)*(2^16-1)));
                    end
                end
                close(vOut)
                close(vOutHighBit)
...
cmd = {srcAfni};
fieldListIn =  {'fBase' 'fRespStat' 'fFit' 'fResid' 'fStat' 'fMat'   };
fieldListOut = {'base'  'respF'     'fit'  'resid'  'stat'  'dsgnMat'};
for i = 1:length(fieldListIn)
    fieldIn = fieldListIn{i};
    fieldOut = fieldListOut{i};
    % if strcmp(fieldOut,'respF'); keyboard; end
    if isfield(f,fieldIn)
        if ~isempty(f.(fieldIn))
            files.(fieldOut).f = reshape(cellstr(f.(fieldIn)),size(f));
        else
            files.(fieldOut).f = [];
        end
    end
...
fieldListIn =  {'fResp' 'fRespOnBase' 'fRespOnBaseMovie' 'fRespOnBaseMovieHighBit'};
fieldListOut = {'resp'  'respOnBase'  'respOnBaseMovie'  'respOnBaseMovieHighBit' };
for i = 1:length(fieldListIn)
    fieldIn = fieldListIn{i};
    fieldOut = fieldListOut{i};
    % if strcmp(fieldOut,'respF'); keyboard; end
    if isfield(f,fieldIn)
        files.(fieldOut).f = cellstr(f.(fieldIn));
    end
    % if nEcho>1 && isfield(fRun_echoRms,fieldIn); dbstack; error('double-check that');
    %     files.(fieldOut).fEchoRms = reshape({fRun_echoRms.(fieldIn)},size(fRun_echoRms));
    % end
    % if nRunOut>1 && ~isempty(fSes) && isfield(fSes,fieldIn) && ~param.skipCat; dbstack; error('double-check that');
    %     files.(fieldOut).fSes = reshape({fSes.(fieldIn)},size(fSes));
    % end
    % if nRunOut>1 && nEcho>1 && isfield(fSes_echoRms,fieldIn) && ~isempty(fSes) && ~param.skipCat; dbstack; error('double-check that');
    %     files.(fieldOut).fSesEchoRms = reshape({fSes_echoRms.(fieldIn)},size(fSes_echoRms));
    % end
    % 
    % %%% catenate echo
    % if nEcho>1; dbstack; error('double-check that');
    ...
    % 
    % %%% catenate runs
    % if nRunOut>1; dbstack; error('double-check that');
    %     if any(ismember(fieldOut,{'base' 'respF'}))
    %         if isfield(fRun,fieldIn)
    %             fIn = reshape({fRun.(fieldIn)},size(fRun));
...
function [f,param] = runAfni(fList,param,fMask,force,verbose)
global srcAfni srcFs
...
for E = 1:size(fList,2)
    ...
    %% Define files
    fIn = fList(:,E);
    tmp = strsplit(fIn{1},'_');
    if all(contains({'sub-' 'ses-'},tmp))
        tmp = char(tmp(contains(tmp,'run-')));
        if size(fList,1)>1
            fOut = replace(fIn{1},tmp,'run-cat');
        end
        if ~exist(fileparts(fOut),'dir'); mkdir(fileparts(fOut)); end
    else
        fOut = replace(fIn{1},'.nii.gz','');
        if ~exist(fOut,'dir'); mkdir(fOut); end
        fOut = fullfile(fOut,'dummy.nii.gz');
    end

    fStat   = fullfile(fileparts(replace(fOut,'.nii.gz','')),'cond-all_stats.nii.gz');
    fMat    = fullfile(fileparts(replace(fOut,'.nii.gz','')),'cond-all_stats.xmat.1D');
    fMatFig = fullfile(fileparts(replace(fOut,'.nii.gz','')),'cond-all_stats.xmat.fig');
    for c = 1:length(param.funDsgn.label)
        fStim{1,c}   = fullfile(fileparts(replace(fOut,'.nii.gz','')),['cond-' param.funDsgn.label{c} '_startTime.1D']);
        fResp{1,c}   = fullfile(fileparts(replace(fOut,'.nii.gz','')),['cond-' param.funDsgn.label{c} '_resp.nii.gz']);
    end
    % fFit    = fullfile(fileparts(replace(fOut,'.nii.gz','')),'cond-all_fit.nii.gz');
    % fResid  = fullfile(fileparts(replace(fOut,'.nii.gz','')),'cond-all_resid.nii.gz');
    fFit    = [];
    fResid  = [];
    ...
    if ~exist(fStat,'file') || force
        cmdTmp = [cmdTmp afniCmd(fIn,fMask,fStim,param.nDummy,param.tr,startSeq,condSeq,param.trDecon,param.funDsgn.label,TENTzeroFlag,fResp,fFit,fResid,fMat,fStat,verbose,param.nDummyRemoved)];

        for c = 1:length(param.funDsgn.label)
            cmdTmp{end+1} = ['echo ''   ''' fResp{c}];
        end
        cmdTmp{end+1} = ['echo ''   ''' fStat];
        cmdTmp{end+1} = ['echo ''   ''' fMat];
        cmdTmp{end+1} = 'echo ''   ''done';
    ...
    f(1,E).cmd =  strjoin(cmdTmp,newline);
    ...
    n = cmd(contains(cmd,'TENT')); n = strsplit(n{1},' '); n = n{contains(n,'TENT')}; n = strsplit(n,{'(' ')'}); n = str2num(n{2}); n = n(3);
    if TENTzeroFlag
        nPoly = size(param.funDsgn.mat,2) - (n-2);
        tStim = (1:n-2).*param.trDecon;
    else
        nPoly = size(param.funDsgn.mat,2) - n;
        tStim = (0:n-1).*param.trDecon;
    end
    tStim = [linspace(tStim(1)-(param.trDecon.*nPoly),tStim(1)-param.trDecon,nPoly) tStim];
    ...
    [~,b,~] = fileparts(fileparts(fIn(1))); b = strsplit(b,'_');
    tmp = strsplit(b{1},'-');
    if strcmp(tmp{1},'sub')
        b{contains(b,'run-')} = 'run-cat'; b = strjoin(b,'_');
    else
        [~,b,~] = fileparts(replace(fIn(1),'.nii.gz',''));
    end
    title(b,'interpreter','none')
    xlabel('time after stim onset (s)')
    ylabel('time after run onset (s)')
...
cmd{end+1} = ['-num_stimts ' num2str(length(label)) ' \'];
...
for k = 1:length(label)
    cmd{end+1} = ['-stim_label ' num2str(k) ' ' char(label{k}) ' \'];

    % write design to file
    fido = fopen(fStim{k}, 'w');
    if iscell(fIn)
        for i = 1:length(fIn)
            fprintf(fido,'%.3f ',startSeq(condSeq==k));
            fprintf(fido,'\n');
        end
    else
        fprintf(fido,'%.3f ',startSeq(condSeq==k));
    ...
    if TENTzeroFlag
        cmd{end+1} = ['-stim_times ' num2str(k) ' ' fStim{k} ' ''TENTzero(' num2str(b) ',' num2str(c) ',' num2str(n) ')'' \'];
    else
        cmd{end+1} = ['-stim_times ' num2str(k) ' ' fStim{k} ' ''TENT(' num2str(b) ',' num2str(c) ',' num2str(n) ')'' \'];
    end
